...
GameObjectFactory::GameObjectFactory(sf::RenderWindow* window)
{
	...
	m_projectileTexture.loadFromFile("Projectile.png");
...
void GameObjectFactory::generateProjectile(std::vector<std::unique_ptr<Projectile>>& projectiles, Player& player)
{
	if (player.launchProjectile())
	{
		projectiles.push_back(std::make_unique<Projectile>(m_projectileTexture));
		projectiles[projectiles.size() - 1].get()->setPosition(player.getPosition());
	}
}
...
void GameObjectFactory::switchDifficulty(int currentHeight)
{
	...
	if (currentHeight > m_prevHeight + m_difficultyHeightGap && m_difficulty != m_maxDifficulty)
	{
		...
		case 5:
		{
			...
			m_bigStaticNPCSpawnChance = 1.2f;
...
std::unique_ptr<GameObject> GameObjectFactory::generateGameObjectByString(const std::string& str)
{
	if (str == "Platform")
	{
